...
struct memseg *addseg(struct memory *mem, rvaddr_t start, rvaddr_t end,
		      uint8_t flags) ATTRIBUTE(nonnull);

// Frees a memory segment
void freeseg(struct memory *mem, struct memseg *seg) ATTRIBUTE(nonnull);
 ...
 */
int memstoreN(struct memory mem, rvaddr_t addr, uint8_t size, const void *in) ATTRIBUTE(nonnull);
